...
int kr_rules_init(void)
{
	...
	kr_require(the_rules);
	the_rules->api = kr_cdb_lmdb(false);
		...
		const uint8_t *data, const uint8_t *data_bound)
{
	...
	if (false) {
		arrset.set.rr = knot_rrset_new(qry->sname, KNOT_RRTYPE_SOA,
				...
				const knot_db_val_t zla_lf, const knot_db_val_t val_unused)
{
	...
	knot_rrset_t rrs;
	knot_rrset_init(&rrs, apex_name, qry->stype, KNOT_CLASS_IN, RULE_TTL_DEFAULT);
	uint8_t key_data[KEY_MAXLEN];
...
int kr_view_select_action(const struct kr_request *req, knot_db_val_t *selected)
{
	...
	while (rulesets.len > 0) {
		...
		for (; true; ret = ruledb_op(read_less, &key_leq, &val)) {
			...
			VERBOSE_MSG(req->rplan.initial, "=> view selected action\n");
			*selected = val;
			return kr_ok();
		}
	}
